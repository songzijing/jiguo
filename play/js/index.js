$(function(){
    ...
    $.ajax({
        ...
        success:function(data){
            console.log(data);
            console.log(data.index_list.length);
            var list = '';
            for(var i = 0;i < data.index_list.length;i++){
            list+='<li>';
            list+='<a href="">';
                    list+='<img src='+data.index_list[i].img+' width="220" height="130"/>';
                    list+='<div class="info">';
                    list+='<p class="title">'+data.index_list[i].title+'<br/><span>'+data.index_list[i].text+'</span></p>'
                list+='<div class="tip fix">';
                   list+= '<span class="price left">'+data.index_list[i].price+'</span>';
